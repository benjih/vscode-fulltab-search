...
import { makeQuery, MARKER } from './testHelpers';
...
suite('SearchEngine Integration Suite', () => {
	...
	test('returns empty results for blank pattern', async function () {
		const results = await engine.search(
			makeQuery({ pattern: '   ' }),
			tokenSource.token
		);
	...
	test('adds breadcrumbs for function context', async function () {
		...
		const results = await engine.search(makeQuery(), tokenSource.token);
		const helloFile = results.fileResults.find((entry) => entry.fileName === 'hello.ts');
		assert.ok(helloFile);
	...
	test('cancels in-flight search', async function () {
		...
		const searchPromise = engine.search(makeQuery({ pattern: MARKER }), cancelSource.token);
		engine.cancel();
		cancelSource.cancel();
