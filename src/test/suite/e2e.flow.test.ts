...
import { makeQuery, MARKER, waitForWebviewPanel } from './testHelpers';

suite('E2E Flow Suite', () => {
	const engine = new SearchEngine();

	suiteSetup(() => {
		assert.ok(vscode.workspace.workspaceFolders?.[0], 'Fixture workspace required');
	});
	...
	test('openMatch navigates editor to match location', async function () {
		...
		const token = new vscode.CancellationTokenSource();
		const results = await engine.search(makeQuery({ include: '**/hello.ts' }), token.token);
		token.dispose();

		const match = results.fileResults[0].matches[0];
		const document = await vscode.workspace.openTextDocument(vscode.Uri.file(match.file));
